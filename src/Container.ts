...
import createTrackableData from "./createTrackableData"
import {
  AnyFn,
  PropEffect,
  TargetMap,
...
class Container<T> {
  ...
  [CONTAINER_EFFECT_MAP]: Map<object, TargetMap>;
  ...
  getTargetEffectMap(target: object) {
  ...
  tryToUpdateUpperPropEffectHasSubProp(target: any) {
    ...
    const trackIdItem = this[TRACK_ID_MAP].get(trackId)
  ...
  tryToTrackEffect({ mapFn, effectHook }: { mapFn: AnyFn; effectHook: AnyFn }) {
    try {
      // track
      const option = {
        proxies: [] as any[],
